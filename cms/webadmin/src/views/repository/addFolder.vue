  ...
  <el-drawer
    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm"  v-loading='loading'>
      <el-form-item
        ...
        class='InpitWidth'>
        <el-input clearable :disabled='ProhibitFromm' autocomplete="off" v-model="ruleForm.fileName" placeholder="请输入文件夹名称"></el-input>
      </el-form-item>
    </el-form>
...
import { findById,insert,update } from '@/api/repository'
...
export default {
  ...
  methods: {
    ...
    initial: function (node) {
      ...
      that.ruleForm.repositoryId = node.repositoryId
      that.ruleForm.parentId = node.parentId
    },
    ...
    async add(){
      const that = this
      let response = await fileInsert(that.ruleForm)
