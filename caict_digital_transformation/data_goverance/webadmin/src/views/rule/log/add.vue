  ...
  <el-drawer
    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm"  v-loading='loading'>
      <el-form-item
        ...
        class='InpitWidth'>
        <el-input clearable :disabled='ProhibitFromm' autocomplete="off" v-model="ruleForm.ruleName" placeholder="请输入规则名称"></el-input>
      </el-form-item>
      <el-form-item
        prop="startTime"
        label="作业开始时间:"
        :label-width="this.formLabelWidth"
        class='InpitWidth'>
        <el-input clearable :disabled='ProhibitFromm' autocomplete="off" v-model="ruleForm.startTime" placeholder="请输入规则名称"></el-input>
      </el-form-item>
      <el-form-item
        prop="status"
        label="执行结果:"
        :label-width="this.formLabelWidth"
        class='InpitWidth'>
        <el-input clearable :disabled='ProhibitFromm' autocomplete="off" v-model="ruleForm.status" placeholder="请输入执行结果"></el-input>
      </el-form-item>
      <el-form-item
        prop="checkCount"
        label="检查条数:"
        :label-width="this.formLabelWidth"
        class='InpitWidth'>
        <el-input clearable :disabled='ProhibitFromm' autocomplete="off" v-model="ruleForm.checkCount" placeholder="请输入检查条数"></el-input>
    </el-form-item>
      <el-form-item
        prop="errorCount"
        label="错误条数:"
        :label-width="this.formLabelWidth"
        class='InpitWidth'>
        <el-input clearable :disabled='ProhibitFromm' autocomplete="off" v-model="ruleForm.errorCount" placeholder="请输入错误条数"></el-input>
      </el-form-item>
      <el-form-item
        prop="logDesc"
        label="日志详情:"  
        :label-width="this.formLabelWidth"
        class='InpitWidth'>
        <el-input type="textarea" clearable :disabled='ProhibitFrom' autocomplete="off" v-model="ruleForm.logDesc" placeholder="请输入日志详情"></el-input>
      </el-form-item>
    </el-form>
...
import { findById } from '@/api/rule/log'
...
export default {
  ...
  methods: {
    ...
    async add(data) {
      ...
      let da = data
      console.log(da)
      let oDate1 = new Date(data.startTime);
      let oDate2 = new Date(data.endTime);
      if(oDate1 > oDate2){
        that.$message.warning('开始时间不能大于结束时间')
        return
      }
      that.loading = true
      const response = await insertQualityReport(da)
      that.loading = false
