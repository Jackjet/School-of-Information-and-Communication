  ...
  <el-main class="main">
     ...
     >
      ...
      <el-form-item
        ...
        class='InpitWidth'>
        <el-select v-model="ruleForm.meta.icon" clearable  :disabled='disabled'  placeholder="请选择菜单图标">
          <el-option
      ...
      <el-form-item label="是否有子菜单:"  prop="child" :label-width="this.formLabelWidth">
        <el-select v-model="ruleForm.child" clearable @change="exchange" :disabled='disabled' placeholder="请选择是否有子菜单">
          <el-option
            v-for="item in menus"
            :key="item.value"
            :label="item.label"
...
export default {
  ...
  created () {
    ...
    if (this.$route.query.type === '新增') {
      ...
      this.ruleForm.component = this.$route.query.component
      this.ruleForm.redirect = this.$route.query.redirect
      this.ruleForm.name = this.$route.query.name
      this.ruleForm.meta = this.$route.query.meta
      this.ruleForm.parentId = this.$route.query.parentId
      this.ruleForm.child = this.$route.query.child
  ...
  methods: {
    exchange() {
      const that = this
      if(that.ruleForm.child === true){
        that.rules.component[0].required = false 
      } else {
        that.rules.component[0].required = true
      }
    },
    submitForm(ruleForm) {
