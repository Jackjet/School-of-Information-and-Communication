...
public class MySQLDatabaseMeta extends BaseDatabaseMeta {
    ...
    public String getSQLQueryFields(String schemaName, String tableName) {
        return String.format("select * from information_schema.columns where table_schema='%s' and table_name='%s'", schemaName, tableName);
    }
    ...
    public String getSQLQueryDataWithPaging(String tableName, int pageIndex, int pageSize) {
        return "select * from " + tableName + " limit " + pageIndex * pageSize + ", " + pageSize;
    }
}
