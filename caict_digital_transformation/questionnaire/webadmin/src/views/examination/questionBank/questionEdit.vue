  ...
  <el-main>
    <el-col :span="24">
      <el-form :inline="true" class='el-InputForm'>
        <el-form-item label="所属分组" label-width="100px" class='InpitWidth'>
          <el-select v-model="examinationGroupId" placeholder="请选择分组">
            <el-option
...
export default {
  ...
  methods: {
    async findGroup () {
      ...
      try {
        ...
        if (response.data.code !== 0) {
          that.itemGroup = response.data.data
        } else {
    ...
    save () {
      ...
      that[that.method](data)
    },
    // 添加方法
    async add (data) {
      const that = this
      let url = that.Interface.courseManage.question.insert
      that.loading = true
