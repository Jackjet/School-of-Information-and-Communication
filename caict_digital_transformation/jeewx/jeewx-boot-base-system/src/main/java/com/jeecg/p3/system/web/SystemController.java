...
import com.jeecg.p3.system.service.*;
...
public class SystemController extends BaseController {
	...
	public void toLogin(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		velocityContext.put("realm",realm);
		velocityContext.put("clientId",clientId);
        velocityContext.put("authBasePath",authBasePath);
...
/*	@RequestMapping(value = "/preLogin", method = { RequestMethod.GET, RequestMethod.POST })
	...
	@RequestMapping(value = "/preLogin", method = { RequestMethod.GET, RequestMethod.POST })
	public void preLogin(String token, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		VelocityContext velocityContext = new VelocityContext();
		String viewName = "base/back/common/prelogin.vm";
		...
		if(token!=null){
			JSONObject auth = authService.getAuthUser(token);
			username = auth.getString("preferred_username");
		}
		...
		try {
			...
			// 验证用户信息
			if (user != null) {
				if (Constants.USER_NORMAL_STATE.equals(user.getUserStat())) {
					isAuth = true;
				} else {
					LOG.info("登录验证失败：用户【" + username + "】登录信息验证不通过");
				}
	...
	public void logout(String jwid, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		request.getSession().removeAttribute(Constants.OPERATE_WEB_LOGIN_USER_TOKEN);
		ViewVelocity.view(request, response, viewName, velocityContext);
	}
...
/*	@RequestMapping(value = "/login", method = { RequestMethod.GET, RequestMethod.POST })
	public void login(String jwid, String username, String password, HttpServletRequest request,
        ...
        //tianjia
		*//*if(1==1) {
			response.sendRedirect("http://47.94.130.160:9001/auth/");
			return;
		}*//*
...
//					LOG.info("menuTree---->"+menuTree);
	...
	@RequestMapping(value = "/login", method = { RequestMethod.GET, RequestMethod.POST })
	public void login(String jwid,String token, HttpServletRequest request,
					  HttpServletResponse response) throws Exception {
...
//					LOG.info("menuTree---->"+menuTree);
				...
				if (user != null) {
						...
						if (jw != null) {
							...
							request.getSession().setAttribute(Constants.SYSTEM_USERID, username);
							request.getSession().setAttribute(Constants.OPERATE_WEB_LOGIN_USER, user);
							request.getSession().setAttribute(Constants.OPERATE_WEB_LOGIN_USER_TOKEN, token);
