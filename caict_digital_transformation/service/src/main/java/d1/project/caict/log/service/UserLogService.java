...
import d1.project.caict.log.dao.UserLogDao;
import d1.project.caict.log.entity.UserLog;
import d1.project.caict.log.mapper.UserLogMapper;
import d1.project.caict.log.model.UserLogExcel;
import d1.project.caict.log.model.vm.UserLogFindAllVm;
...
public class UserLogService {
    ...
    public void insert(UserLogRequestInsertVm model, HttpServletRequest request) throws DoValidException {
        ...
        UserLog userLog = mapper.dtoFormatIntoRequestInsertEntity(model);
     ...
     * @param model 实体
     */
    public void insert(UserLogRequestInsertVm model, WebAdminUser webAdminUser, HttpServletRequest request) {
        UserLog userLog = mapper.dtoFormatIntoRequestInsertEntity(model);
        userLog.setId(BaseUtils.generate32Id());
        userLog.setSourceIp(BaseUtils.getIpAddress(request));
        /*0->系统,1->人为*/
     ...
     * 查询所有
     */
    public Page<UserLog> findAll(UserLogFindAllVm model, HttpServletRequest request) throws Exception {
        JSONObject paramObject = (JSONObject) JSON.toJSON(model);
        myTokenService.makeUpRoleJsonObjectByToken(request, paramObject);
        SpecificationBuilder<UserLog> builder = new SpecificationBuilder<>();
        Specification<UserLog> specification = getUserLogSpecification(paramObject, builder);
    ...
    public List<UserLogExcel> findAllList(UserLogFindAllVm model, HttpServletRequest request) throws Exception {
        ...
        SpecificationBuilder<UserLog> builder = new SpecificationBuilder<>();
        Specification<UserLog> specification = getUserLogSpecification(paramObject, builder);
        List<UserLog> all = userLogDao.findAll(specification);
     ...
     * 获取查询规则
     */
    private Specification<UserLog> getUserLogSpecification(JSONObject paramObject, SpecificationBuilder<UserLog> builder) throws Exception {
     ...
     * @param id id
     */
    public Optional<UserLog> find(String id) {
