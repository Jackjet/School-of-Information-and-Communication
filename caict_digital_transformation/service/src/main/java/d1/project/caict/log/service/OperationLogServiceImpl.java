package d1.project.caict.log.service;
...
import d1.project.caict.common.service.MyTokenService;
import d1.project.caict.common.utils.BaseUtils;
import d1.project.caict.log.dao.OperationLogDao;
import d1.project.caict.log.entity.OperationLog;
import d1.project.caict.log.mapper.OperationLogMapper;
import d1.project.caict.log.model.OperationLogExcel;
import d1.project.caict.log.model.vm.OperationLogFindAllVm;
...
public class OperationLogServiceImpl implements IOperationLogService {
     ...
     */
    @Override
    public void insert(d1.project.caict.common.model.OperationLog model, HttpServletRequest request) throws DoValidException {
        JSONObject userByToken = TokenManager.getInstance().getUserByToken(request);
        OperationLog operationLog = mapper.dtoFormatIntoRequestInsertEntity(model);
     ...
     * 查询所有(分页)
     */
    public Page<OperationLog> findAll(OperationLogFindAllVm model, HttpServletRequest request) throws Exception {
        ...
        Specification<OperationLog> specification = getUserLogSpecification(paramObject, builder);
    ...
    public List<OperationLogExcel> findAllList(OperationLogFindAllVm model, HttpServletRequest request) throws Exception {
        ...
        SpecificationBuilder<OperationLog> builder = new SpecificationBuilder<>();
        Specification<OperationLog> specification = getUserLogSpecification(paramObject, builder);
        List<OperationLog> all = operationLogDao.findAll(specification);
     ...
     * 获取查询规则
     */
    private Specification<OperationLog> getUserLogSpecification(JSONObject paramObject, SpecificationBuilder<OperationLog> builder) throws Exception {
    ...
    public void export(OperationLogFindAllVm model, HttpServletResponse response, HttpServletRequest request) throws Exception {
        ...
        insert(new d1.project.caict.common.model.OperationLog("日志管理_操作日志", "导出", "导出操作日志", JSON.toJSONString(model), 1), request);
    }
