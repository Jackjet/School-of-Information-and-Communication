...
//---------------------------------------------------动名词状语从句------------------------------------------------------------
            ...
            case DATETIME_THAN:
                if (!StringUtils.isEmpty(param)) {
                    try {
                        ...
                        if (time != null) {
                            predicate = criteriaBuilder.greaterThan(rootKey, time);
                        }
                    } catch (Exception e) {
            e.printStackTrace();
                        //无需处理异常
            ...
            case DATETIME_LESS:
                if (!StringUtils.isEmpty(param)) {
                    try {
                        ...
                        Date time = timeFormat.parse(param);
                        if (time != null) {
                            predicate = criteriaBuilder.lessThan(rootKey, time);
                        }
                    } catch (Exception e) {
            e.printStackTrace();
                        //无需处理异常
            ...
            case DATETIME_THAN_EQUAL:
                if (!StringUtils.isEmpty(param)) {
                    try {
                        ...
                        Date time = timeFormat.parse(param);
                        if (time != null) {
                            predicate = criteriaBuilder.greaterThanOrEqualTo(rootKey, time);
                        }
                    } catch (Exception e) {
            e.printStackTrace();
                        //无需处理异常
            ...
            case DATETIME_LESS_EQUAL:
                if (!StringUtils.isEmpty(param)) {
                    try {
                        ...
                        Date time = timeFormat.parse(param);
                        if (time != null) {
                            predicate = criteriaBuilder.lessThanOrEqualTo(rootKey, time);
                        }
                    } catch (Exception e) {
            e.printStackTrace();
                        //无需处理异常
    ...
    private Predicate datetimeBetween(CriteriaBuilder criteriaBuilder, SpecificationCondition condition, Path rootKey) {
        ...
        if (startTime == null) {
            if (endTime != null) {
                predicate = criteriaBuilder.lessThan(rootKey, endTime);
            }
        } else {
            if (endTime == null) {
                predicate = criteriaBuilder.greaterThanOrEqualTo(rootKey, startTime);
            } else {
                predicate = criteriaBuilder.between(rootKey, startTime, endTime);
