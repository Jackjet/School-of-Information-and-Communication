...
import { getJsonTreeAssiagn, getJsonTreeChoose } from '@/utils/tool'
export default {
  data () {
    return {
      id: '',
  ...
  methods: {
    ...
    async getList () {
      ...
      try {
        ...
        if (response.data.code === 1) {
          ...
          that.dataArray = getJsonTreeAssiagn(JSON.parse(arrTree), '')
          that.choose = getJsonTreeChoose(JSON.parse(arrChoose), '')
          this.findCurrent()
    ...
    async findCurrent () {
      ...
      try {
        ...
        that.isSubmitLoading = false 
        if (response.data.code === 1) {
          that.choose = that.choose.concat(response.data.data)
    ...
    onPageReturn() {
      this.$router.push({ path: '/systemManage/role/roles' })  
    } 
  }
}
</script>
