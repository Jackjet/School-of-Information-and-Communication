...
<script>
import { rolePermissionFindAll, rolePermissionInsert } from '@/api/websiteManage'
export default {
  data () {
    return {
      id: '',
  ...
  methods: {
    renderContent(h, { node, data }) {
      let dom;
      // 懒加载图标设置
      dom = (<p class='custom-tree-node'><img src={require('@/assets/image/wj.png')}></img><span style='margin-left:5px;' title={data.desc}>{node.label} </span> </p>);
      return dom;
    },
    TreeAllChange (val) {
      let that = this
    ...
    async getFindList () {
      ...
      try {
        ...
        const response = await rolePermissionFindAll({ roleId : this.id })
        if (response.code === 1) {
          let arr = []
    ...
    async onSubmit () {
      ...
      try {
        ...
        const response = await rolePermissionInsert(obj)
        if (response.data.code === 1) {
          that.isSubmitLoading = false
          that.$message.success(response.data.msg)
          this.$router.push({ path: '/websiteManage/role/index' })
          // that.getList()
          return false
        }
        that.$message.error(response.msg)
