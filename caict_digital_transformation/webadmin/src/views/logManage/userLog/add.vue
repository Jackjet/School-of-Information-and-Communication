<template>
  <el-main class="main">
     ...
     >
      ...
      <el-form-item label="来源IP:" :label-width="this.formLabelWidth" >
        <el-input  maxlength = '100'  placeholder="请输入来源IP" clearable size="medium" :disabled="disabledo" v-model="ruleForm.sourceIp"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async userDetail() {
      ...
      that.loading = true
      const response = await userLogFind(data)
      that.loading = false
    ...
    CloseModal() {
      ...
      this.$router.push({ 
        path: '/logManage/userLog/userLogs'
      })
    },
  }
}
